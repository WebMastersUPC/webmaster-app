  ...
  <pv-card class="w-8 border-round-3xl shadow-4" aria-label="Deliverable Card">
    <template #header>
      <div class="flex flex-row justify-content-center mt-4 -mb-4 gap-5 mx-4 overflow-hidden"
           aria-label="Deliverable Header">
        <h2 v-if="!isEditingTitle">{{ deliverable.title }}</h2>
        <pv-textarea style="width: 70%; height: 70%;" v-else v-model="deliverable.title" autoResize
                     aria-label="Title Textarea"></pv-textarea>
        <pv-button @click="toggleEditingTitle" icon="pi pi-pencil" class="icons" v-if="!isEditingTitle"
                   aria-label="Edit Title Button"></pv-button>
        <pv-button @click="toggleEditingTitle" icon="pi pi-check" class="icons" v-else
                   aria-label="Save Title Button"></pv-button>
      </div>
    </template>
    ...
    <template #title>
      <p class="-mb-1" aria-label="Description Label">
        ...
        <pv-button @click="toggleEditingDescription" icon="pi pi-pencil" class="icons" v-if="!isEditingDescription"
                   aria-label="Edit Description Button"></pv-button>
        <pv-button @click="toggleEditingDescription" icon="pi pi-check" class="icons" v-else
                   aria-label="Save Description Button"></pv-button>
      </p>
    </template>
      ...
      <div class="flex flex-row justify-content-between" aria-label="Deliverable Content">
        <div class="text-container overflow-hidden flex flex-column w-10 h-5" aria-label="Description Content">
          ...
          <pv-textarea style="width: 100%; height: 100%;" v-else v-model="deliverable.description"
                       aria-label="Description Textarea"></pv-textarea>
        </div>
        <div class='flex flex-column mr-5 -mt-6' aria-label="Additional Information">
          <div aria-label="Delivery Date Information">
            ...
            <div class="flex flex-column align-items-center -mt-3 ml-5">
              <p>{{ new Date(deliverable.deadline).toLocaleDateString() }}</p>
            </div>
          </div>
